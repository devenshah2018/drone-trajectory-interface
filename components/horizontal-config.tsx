...
export interface HorizontalConfigRef {
  ...
  /** Apply a mission preset by key and sync sidebar selection */
  applyMissionPreset: (key: string) => void;
  /** Apply a full template (camera + drone + mission) by key */
...
export const HorizontalConfig = forwardRef<HorizontalConfigRef, HorizontalConfigProps>(
  ({ camera, datasetSpec, onCameraChange, onDatasetSpecChange, droneConfig, onDroneChange, planGenerated, variant = "default" }, ref) => {
    ...
    const [globalConfigs] = useState<Record<string, GlobalConfigType>>(templateConfigs as Record<string, GlobalConfigType>);
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        getCameraPreset,
        getMissionPreset,
        applyMissionPreset: (key: string) => {
          const preset = missionPresets[key];
          if (!preset) return;
          setSelectedMissionPreset(key);
          setEditedMissionFields(new Set());
          onDatasetSpecChange({ ...preset.config });
        },
        applyTemplate: (key: string) => {
          handleGlobalConfigSelect(key);
        },
      }),
      [droneVMax, droneAMax, camera, datasetSpec, selectedPreset, selectedMissionPreset, onDatasetSpecChange]
    );
